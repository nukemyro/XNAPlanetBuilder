...
texture NightMap;
sampler NightMapSampler = sampler_state
{
	texture = <NightMap>;
	MinFilter = Anisotropic; //minification factor
	MagFilter = Anisotropic; //magnifiaction factor
...
VS_OUT VS_Color(VS_IN input)
{
	...
	float h = tex2Dlod(heightMapSampler, float4(input.UV, 0, 0)).r * 0.5;
...
float4 PS_Pass(VS_OUT input) : COLOR0
{	
...
{	
	...
	float3 night = tex2D(NightMapSampler, input.UV);			
...
technique t1
{
	pass wire
	{
		...
		PixelShader = compile ps_3_0 PS_Pass();
	}
}